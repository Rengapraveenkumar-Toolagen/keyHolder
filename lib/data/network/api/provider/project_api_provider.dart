...
class ProjectApiProvider {
  ...
  Future<Response> getAllProjects(
      int projectPage, int projectLimit, String searchProject) async {
    try {
      final Response response = await _dioClient.get(
          '${Endpoints.getAllProjects}?page=$projectPage&limit=$projectLimit&keyword=$searchProject');
      return response;
    } catch (error) {
      rethrow;
    }
  ...
  Future<Response> createProject(String projectName, String userId) async {
    try {
      var response = await _dioClient.post(Endpoints.createProject,

...
cp "$PATH_PROJECT/build/app/outputs/flutter-apk/app-dev-release.apk" "$PATH_PROJECT/prod_app $(date '+%Y-%m-%d %H-%M-%S').apk"
# move file app-release.aab to root folder
cp "$PATH_PROJECT/build/app/outputs/bundle/devRelease/app-dev-release.aab" "$PATH_PROJECT/prod_app dev $(date '+%Y-%m-%d %H-%M-%S').aab"